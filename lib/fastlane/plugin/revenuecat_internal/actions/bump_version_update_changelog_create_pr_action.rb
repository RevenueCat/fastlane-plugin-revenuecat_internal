  ...
  module Actions
    class BumpVersionUpdateChangelogCreatePrAction < Action
      ...
      def self.run(params)
        ...
        # For releases older than the latest published version, we need to create a new PR against main to update the changelog with the changes from this release
        ...
        if is_older_than_latest_published_version
          ...
          if dry_run
            ...
            UI.important("Creating a new PR against main to update the changelog with the changes from this release")
          end
          ...
          Helper::RevenuecatInternalHelper.insert_old_version_changelog_in_main(new_version_number, generated_contents, changelog_path)

          if dry_run
            updated_changelog = File.read(changelog_path)
            UI.important("Dry run mode enabled. No PR will be made. The updated changelog in main with contents from this release would look like this:\n#{updated_changelog}")
            Helper::RevenuecatInternalHelper.discard_changes_in_current_branch

            # Checkout the current branch to let the next steps continue in dry_run mode
            ...
            # Checkout the new release branch to let the next steps continue
            Helper::RevenuecatInternalHelper.create_or_checkout_branch(new_branch_name)
          end
        end
      end
